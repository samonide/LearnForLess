...
- Bug Fix Wave 1 (2026-08-22, from FoundBugs.md — C1-A, C1-B, C2, H2):
  - C1-A/C1-B: `redeem_access_token` hardened in migration 008 (`supabase/migrations/008_redeem_token_hardening.sql`, MUST be applied to hosted Supabase via SQL Editor): auth.uid() guard (NULL-safe) + EXECUTE revoked from anon/public, granted to authenticated only; profile upsert reduced to insert-if-missing (`ON CONFLICT DO NOTHING`) so redemption never renames students, nulls emails, or changes roles. Caller `redeemTokenAuthenticated()` now invokes the RPC with the student's own session client (service-role client would fail the guard). Repair SQL for token-renamed profiles included as commented queries in migration 008.
  - C2: `next.config.ts` sets `experimental.serverActions.bodySizeLimit: "525mb"` (default was 1MB) — unblocks 500MB lesson uploads, course .db imports, thumbnails.
  - H2: parser deduplicates exact duplicate source rows (warning emitted per group) and assigns deterministic material-disambiguated fingerprints to same-key/different-material rows; import actions skip+warn on `lessons_source_fingerprint_unique` violations instead of aborting, in all three modes (new/incremental/replacement); rollback preserved for genuine errors. Real `DBTest/apna_videos.db`: 46 byte-identical rows dropped across 33 warnings, 6 operator lessons kept via disambiguation, 785 unique fingerprints.
  - Verification: unit 17/17, integration importer-import 4/4 + importer-reimport 13/13 + auth-flow 23/23 against hosted DB (hardening/identity assertions auto-skip until migration 008 applied), `tsc --noEmit` ✅, `npm run build` ✅. Importer integration tests given 300–600s timeouts (full-course import ≈90–120s of RPC round-trips).
  - Known remaining: C3 (thumbnail URLs), C4 (signup metadata), H1/H3+ per FoundBugs.md; old live function still active until 008 is applied. (008 has since been applied and verified.)
- Bug Fix Wave 2 (2026-08-22, from FoundBugs.md — C3, C4, H1, H3–H7, M2–M7, M10):
  - C3: migration 010 creates public `course-thumbnails` bucket (10MB, image-only, admin + public-read policies) and nulls legacy broken private-bucket thumbnail URLs; `uploadCourseThumbnail()` uploads there, stores the now-valid public URL, removes stale objects on extension change.
  - C4: migration 009 hardcodes `'student'` in `handle_new_user()` (trigger recreated); dashboard "Allow new users to sign up" setting still worth checking manually.
  - H1: replacement re-import snapshots `lesson_progress` before delete and re-links rows to recreated lesson ids via source fingerprints (dedup merge, chunked upserts, rollback restores originals). `ImportResult.progressRestored` surfaced in UI; replacement dialog copy updated.
  - H3: `getCourseForViewer()` returns `enrolled`; viewer hides Mark-as-Complete for preview visitors with an explanatory note; `markLessonComplete()` pre-checks enrollment server-side with actionable error instead of raw RLS failure.
  - H3 root cause found + FIXED W3 (2026-08-23): progress read used `.in("lesson_id", allLessonIds)` — on Prime 2.0 (785 lessons) the ~35 KB URL got HTTP 400 from Supabase and the ignored error silently emptied the progress map, so completed lessons rendered as uncompleted (writes were always fine). Replaced with a single embedded `modules → lessons → lesson_progress` fetch (RLS-scoped, request size independent of course length). Verified end-to-end in real browser: revisit shows completed state, click→auto-advance sidebar updates instantly, persists across reload, course page reads "2 of 785 lessons completed".
  - H4: lesson file replacement uploads + links new object before deleting old. H5: reorder actions fail loudly on any per-item error (per-statement updates retained; retry idempotent). H6: lesson editor shows read-only Imported-source panel (stream URL / B2 key / backup link) and preserves `content` on same-type media saves. H7: recovery-token claim update chains `.is("used_at", null)` + `.select()`; zero rows ⇒ `recovery_token_used`.
  - M2: slug fallback for non-Latin titles. M3: admin search quotes/escapes `.or()` values. M4: progress numbers clamped 0–100 / ≥0 server-side. M5: media lessons require a source in validate() + `updateLesson()` backstop rejects zero-source media saves. M6: type switches/removals clear stale `storage_path` (+ orphan object deleted server-side) only when no replacement upload follows; imported `external_*` preserved. M7: `getLessonContent()` binds lesson to URL courseId. M10: dead `src/components/course-builder.tsx` deleted.
  - NOT addressed (needs info): M1 PDF env/CORS/CDN diagnostics (code path already correct), M8 dashboard layout repro details.
  - Verification: `tsc --noEmit` ✅, `npm run build` ✅. Full test suite intentionally deferred until all bug-hardening waves finish. Migrations 009 + 010 MUST be applied via SQL Editor.
